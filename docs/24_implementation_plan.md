...
5. [✓] [ВЛ] Решение АДР-001: **база данных**. Выбрать **PostgreSQL 16** (реляционные данные: сессии, кэш, состояния, логи, метрики). → **выполнено**: карточка `ADR-001` в [`decisions.md`](decisions.md#adr-001) (контекст, решение, последствия, альтернативы; В-3, ПРОТ-18). Реализация — ФАЗА 2, шаги 81–170.
6. [✓] Обоснование АДР-001: нужен SQL (агрегации метрик, TTL-чистки, индексы, транзакции сессий); KV-хранилище не даёт запросов к метрикам. → **выполнено**: блок «Обоснование» (требование → механизм) и «Почему не KV-хранилище» в карточке `ADR-001`.
7. [✓] Альтернативы АДР-001: H2 (только тесты), Redis (только кэш/сессии, не метрики) — зафиксировать «почему нет». → **выполнено**: блок «Альтернативы» в `ADR-001` — H2, Redis, MySQL 8, ClickHouse/TimescaleDB.
8. [✓] Решение АДР-002: **SQL-доступ**. Использовать **Spring Data JDBC + JdbcTemplate** (без JPA: сущности источников не энтити БД). → **выполнено**: карточка `ADR-002` ([`decisions.md`](decisions.md#adr-002)); уточнено, что берётся `spring-boot-starter-jdbc` + `NamedParameterJdbcTemplate`, без модуля `spring-data-jdbc`. Реализация — ФАЗА 2 (шаги 95–103).
9. [✓] Обоснование АДР-002: доменная модель (Offer и сущности источников) отделена от таблиц; JPA-магия принесёт больше проблем. → **выполнено**: блок «Обоснование» (требование → механизм `JdbcTemplate`, 8 строк) и «Альтернативы» (JPA, Spring Data JDBC, MyBatis, jOOQ, чистый `JdbcTemplate`).
10. [✓] Решение АДР-003: **миграции**. Flyway (`flyway-core`) как единственный инструмент изменения схемы. → **выполнено**: карточка `ADR-003` + блоки «Обоснование»/«Альтернативы». Реализация — шаги 88, 90, 128–129.
11. [✓] Решение АДР-004: **тестовая БД**. H2 в PostgreSQL-режиме для юнит/интеграционных тестов; `Testcontainers` — опционально для pg-специфики. → **выполнено**: карточка `ADR-004` + блоки «Обоснование»/«Альтернативы». Реализация — шаги 84, 87, 114, 138–139.
12. [✓] [ВЛ] Решение АДР-005: **кэш и сессии поверх Postgres** через таблицы (без внешних Redis-зависимостей на первом этапе). [ВЛ] Подтверждено. → **выполнено**: карточка `ADR-005` + блоки «Обоснование»/«Альтернативы». Реализация — ФАЗА 3 (`session`/`session_message`/`cached_result`) и ФАЗА 14.
