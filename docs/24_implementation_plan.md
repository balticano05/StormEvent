...
58. Создать `ApiResponseEnvelope` (не обязательно) — если решим единый конверт; ADR-018.
...
71. ADR-019: RequestContext передаётся явно (память о параллелизме), MDC — только для логов.
...
96. ADR-020: используем `NamedParameterJdbcTemplate` — безопасность от SQL-инъекций, именованные параметры.
...
102. ADR-021: все запросы к БД — через репозитории (`repository/` пакет), не в сервисах.
...
108. ADR-022: выбранная схема БД — `storm` (изоляция от других приложений).
...
119. ADR-023: JSON в jsonb, никогда не фильтруется в SQL — извлекается в приложении.
...
126. ADR-024: аспект мониторинга репозиториев через `@Timed`-аналог (AOP), не меняет вызовы.
...
133. ADR-025: транзакции только на уровне сессии/кэша/логов; чтение источников — без транзакций.
...
146. ADR-026: фоновые джобы НЕ держат транзакцию во время сетевых вызовов источников.
...
153. ADR-027: таблицы логов партицируются по времени (V-миграции позже, фаза 4).
...
162. ADR-028: если jsonb мешает H2, то в тестах маппинг через `JsonNode` из TEXT — фикс-маршрут.
...
233. ADR-033: партиционирование только на prod-pg через заданную миграцию; тесты используют полную таблицу.
...
251. ADR-035: границы агрегации — UTC-часы.
...
257. ADR-036: pessimistic locking на session при refine (короткая транзакция).
...
275. ADR-037: FK-колонки всегда индексируются, если левая часть не покрыта PK.
...
302. ADR-039: целевой budget: поиск сессии <5 мс, upsert кэша <10 мс, housekeeper <100 мс пачкой.
...
316. ADR-040: конфиг БД управляется через миграции + переменные докера, без ручного ALTER.
...
340. ADR-042: кэш — нормализованные офферы (jsonb), не сырые HTML.
...
350. ADR-043: housekeeper использует `SELECT ... FOR UPDATE SKIP LOCKED` на малых пачках.
...
429. ADR-046: user-messages хранятся в i18n (ru/en), коды стабильны.
...
460. ADR-047: курс руб/долл в Constants как первичная метрика, обновляется вручную + `currency.unknown` флаг.
...
496. ADR-048: нормализация выполняется один раз на гейтвее, не повторно.
...
533. ADR-049: таймауты источников наследуют глобальные + overrides в `source-*.timeout-ms`.
...
580. ADR-050: сброс circuit при повторном включении источника (после drain) разрешён.
...
719. ADR-051: rank-модель простая (v1), score формула фиксируется здесь.
